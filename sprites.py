...
class Ball(pygame.sprite.Sprite):
    ...
    def draw(self, surf):  # ball drawing method
        ...
        pygame.draw.arc(surf, (220, 100, 20),  # second arc
                        (self.pos.x - self.r, self.pos.y - self.r // 2, self.r * 2, self.r),
                        math.radians(210), math.radians(330), 2)  # second arc

    def backboard_collision(self, backboard_rect):
        # nearest point on backboard to ball
        nearest_x = max(backboard_rect.left, min(self.pos.x, backboard_rect.right))
        nearest_y = max(backboard_rect.top, min(self.pos.y, backboard_rect.bottom))
